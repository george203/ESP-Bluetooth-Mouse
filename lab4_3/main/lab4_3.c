...
#define HIDD_DEVICE_NAME            "HID_George"
...
void hid_demo_task(void *pvParameters)
{
    ...
    while(1) {
        ...
        if(read_register(GYRO_XOUT_H, data, 6) == ESP_OK) {
            ...
            if(gyro_y > 300) {
                if(gyro_x > 300) {
                    ESP_LOGI(HID_DEMO_TAG, "MOUSE UP LEFT");
                    x_value = -5;
                    y_value = -5;
                }
                else if(gyro_x < -300) {
                    ESP_LOGI(HID_DEMO_TAG, "MOUSE UP RIGHT");
                    x_value = 5;
                    y_value = -5;
                }
                else {
                    ESP_LOGI(HID_DEMO_TAG, "MOUSE UP");
                    y_value = -5;
                }
            }
            else if(gyro_y < -300) {
                if(gyro_x > 300) {
                    ESP_LOGI(HID_DEMO_TAG, "MOUSE DOWN LEFT");
                    x_value = -5;
                    y_value = 5;
                }
                else if(gyro_x < -300) {
                    ESP_LOGI(HID_DEMO_TAG, "MOUSE DOWN RIGHT");
                    x_value = 5;
                    y_value = 5;
                }
                else {
                    ESP_LOGI(HID_DEMO_TAG, "MOUSE DOWN");
                    y_value = 5;
                }
            }
            else {
                if(gyro_x > 300) {
                    ESP_LOGI(HID_DEMO_TAG, "MOUSE LEFT");
                    x_value = -5;
                }
                else if(gyro_x < -300) {
                    ESP_LOGI(HID_DEMO_TAG, "MOUSE RIGHT");
                    x_value = 5;
                }
                else {
            ...
            if((gyro_x < -800) || (gyro_x > 800)) {
                x_value = x_value * 3;
            }
            if((gyro_y < -800) || (gyro_y > 800)) {
                y_value = y_value * 3;
            }
        }
        if (sec_conn) {
            ESP_LOGI(HID_DEMO_TAG, "Send mouse movement");
            if(click == 1) {
                ESP_LOGI(HID_DEMO_TAG, "CLICK");
                esp_hidd_send_mouse_value(hid_conn_id, 0x01, 0, 0);
                vTaskDelay(5 / portTICK_PERIOD_MS);
            }
            esp_hidd_send_mouse_value(hid_conn_id, 0x00, x_value, y_value);
        }
